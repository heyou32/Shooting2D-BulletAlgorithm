...
public class PageEngine
{
    ...
    public static float ExecuteScript(Transform transform, Vector2 sizeDelta, string script, AssetBank assets, bool allCaps = false, string dataBreak = "&", string lineBreak = "|")
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            else if (line.Length == 2)
            {
                ...
                {
                    var gameObject = assets.GetGameObject(data[0]);
                    if (gameObject == null) break;

                    var size = _GetSize(gameObject);
                ...
                {
                    var gameObjects = line[1].Split(","[0]);
                    if (gameObjects == null) break;

                    var height = _AddGameObjects(transform, assets, gameObjects, x, y, gameObjectRowSpacing, scale, rotation);
    ...
    static float _AddGameObjects(Transform transform, AssetBank assets, string[] gameObjects, float x, float y, float space, float scale, float rotation)
    {
        ...
        {
            var gameObject = assets.GetGameObject(gameObjects[i]);
        ...
        var newGameObject = new GameObject[gameObjects.Length];
        ...
        {
            var size = _GetSize(newGameObject[0]);
        ...
        for (int i = 0; i < gameObjects.Length; i++)
        {
            ...
            var size = _GetSize(newGameObject[i]);
    ...
    static Vector3 _GetSize(GameObject gameObject)
    {
        ...
        var rectangles = gameObject.GetComponentsInChildren<RectTransform>();
        if (rectangles.Length == 0) return size;

        for (int i = 0; i < rectangles.Length; i++)
        {
            var x = Mathf.Abs(rectangles[i].localPosition.x);
            x += rectangles[i].rect.width / 2;
            x *= 2;

            var y = Mathf.Abs(rectangles[i].localPosition.y);
